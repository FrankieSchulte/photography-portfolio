(() => {
  ...
  function input(label, path, options = {}) {
    ...
    return `<label class="field${wide}"><span>${escapeHtml(label)}</span><input data-path="${escapeHtml(path)}" type="${escapeHtml(type)}" value="${escapeHtml(value)}"${options.min !== undefined ? ` min="${options.min}"` : ""}${options.max !== undefined ? ` max="${options.max}"` : ""}${options.step !== undefined ? ` step="${options.step}"` : ""}>${help}</label>`;
  }
  ...
  function renderBasics() {
    ...
    main.innerHTML = sectionHead("Site basics", "Change your name, contact details, navigation labels, and development safety settings.") +
      panel("Identity & contact", `<div class="grid">${input("Name", "meta.name")}${input("Short mark", "meta.shortName")}${input("Footer ASCII artwork", "meta.footerAscii", { type: "textarea", wide: true, help: "Use as many centered lines and blank lines as the artwork needs. Spacing is preserved." })}${input("Footer ASCII line spacing", "meta.footerAsciiLineHeight", { type: "number", min: .7, max: 3, step: .05, help: "1 is tight; larger numbers add more vertical air." })}${input("Role", "meta.role")}${input("Location", "meta.location")}${input("Email", "meta.email", { type: "email" })}${input("Instagram label", "meta.instagramLabel")}${input("Instagram URL", "meta.instagramUrl", { wide: true })}${input("Site description", "meta.siteDescription", { type: "textarea", wide: true })}</div>`) +
  ...
  function renderHome() {
    const slides = state.home.heroSlides.map((slide, index) => `<div class="list-row"><div class="row-fields">
      ...
      ${input("Destination shoot link", `home.heroSlides.${index}.href`, { wide: true, help: "Example: /work/concerts/live-room-demo/" })}
    ...
    main.innerHTML = sectionHead("Landing page", "The homepage uses a slowly moving editorial contact sheet, your introduction, and a clear route into the archive.") +
      panel("Opening copy", `<div class="grid">${input("Small label", "home.eyebrow", { wide: true })}${input("Main headline (metadata fallback)", "home.headline", { wide: true, help: "The visible masthead uses your name from Site basics." })}${input("Short introduction", "home.intro", { type: "textarea", wide: true })}${input("Primary button name", "home.primaryCta.label")}${input("Primary button link", "home.primaryCta.href")}${input("Secondary button name", "home.secondaryCta.label")}${input("Secondary button link", "home.secondaryCta.href")}</div>`) +
      panel("Animated ASCII background", `<div class="notice">Paste monospaced ASCII or Unicode Braille artwork here. Spacing and line breaks are preserved. Individual characters twinkle; on very wide screens the artwork is mirrored on the opposite side. Reduced-motion visitors see a still version.</div><div class="grid" style="margin-top:.8rem">${input("ASCII artwork", "home.asciiArt", { type: "textarea", wide: true })}${input("Twinkle pulses per second", "home.twinkleFrequency", { type: "number", min: 1, max: 30, step: 1, help: "Higher values flicker more frequently." })}${input("Characters per pulse", "home.twinkleAmount", { type: "number", min: 1, max: 80, step: 1, help: "How many marks brighten at the same time." })}${input("Twinkle duration (milliseconds)", "home.twinkleDuration", { type: "number", min: 100, max: 5000, step: 50, help: "How long each bright mark remains visible." })}${input("Twinkle brightness (%)", "home.twinkleBrightness", { type: "number", min: 20, max: 100, step: 1, help: "100 is solid black; lower values are subtler." })}</div>`) +
      panel("Featured horizontal sequence", `<div class="notice">The contact sheet advances slowly on its own and still supports manual scrolling. Empty frames stay hidden until you upload an image. Choose the frame shape that matches each photograph so the original composition is preserved.</div><div class="row-list" style="margin-top:.8rem">${slides}</div>`, '<button class="button small secondary" type="button" data-action="add-slide">+ Add frame</button>');
  }
  ...
  function renderAbout() {
    ...
    main.innerHTML = sectionHead("About & inquiry", "Keep the writing first-person and specific. The form creates a real email draft instead of pretending it sent anything.") +
      ...
      panel("Inquiry page", `<div class="grid">${input("Small label", "inquire.eyebrow", { wide: true })}${input("Headline", "inquire.headline", { wide: true })}${input("Introduction", "inquire.intro", { type: "textarea", wide: true })}${input("Formspree endpoint", "inquire.formEndpoint", { wide: true, help: "Paste the endpoint from your Formspree form, such as https://formspree.io/f/abcdwxyz." })}${input("Submit button name", "inquire.buttonLabel", { wide: true })}</div><div class="row-list" style="margin-top:.8rem">${typeRows}</div>`, '<button class="button small secondary" type="button" data-action="add-project-type">+ Add project type</button>');
  }
  ...
  function handleAction(action, element) {
    ...
    if (action === "add-slide") state.home.heroSlides.push({ src: "", alt: "", position: "50% 50%", orientation: "portrait", label: "Selected frame", href: "/work/" });
